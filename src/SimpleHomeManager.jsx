...
const FOOD_DATABASE = [
  ...
  { keys: ["pizza slice", "slice of pizza"], calories: 285, protein: 12, carbs: 36, fat: 10 },
  { keys: ["pizza"], calories: 850, protein: 36, carbs: 105, fat: 32 },
  { keys: ["chicken breast", "cooked chicken", "chicken"], calories: 165, protein: 31, carbs: 0, fat: 4, basisGrams: 100 },
  { keys: ["steak", "beef"], calories: 250, protein: 26, carbs: 0, fat: 17, basisGrams: 100 },
  { keys: ["salmon"], calories: 208, protein: 20, carbs: 0, fat: 13, basisGrams: 100 },
  { keys: ["tuna can", "can of tuna"], calories: 150, protein: 32, carbs: 0, fat: 2 },
  { keys: ["rice"], calories: 205, protein: 4, carbs: 45, fat: 0, serving: "cup" },
  { keys: ["pasta"], calories: 500, protein: 18, carbs: 82, fat: 12 },
  { keys: ["naan"], calories: 260, protein: 8, carbs: 45, fat: 6 },
  { keys: ["lebanese bread", "pita bread"], calories: 180, protein: 6, carbs: 35, fat: 2 },
  { keys: ["bread", "toast"], calories: 90, protein: 3, carbs: 17, fat: 1 },
  { keys: ["wrap", "tortilla"], calories: 200, protein: 6, carbs: 35, fat: 5 },
  { keys: ["egg", "eggs"], calories: 72, protein: 6, carbs: 0, fat: 5 },
  { keys: ["banana"], calories: 105, protein: 1, carbs: 27, fat: 0 },
  { keys: ["apple"], calories: 95, protein: 0, carbs: 25, fat: 0 },
  { keys: ["oats", "oatmeal"], calories: 380, protein: 13, carbs: 68, fat: 7, basisGrams: 100 },
  { keys: ["whey", "protein shake", "protein scoop"], calories: 120, protein: 24, carbs: 3, fat: 2 },
  { keys: ["yoghurt", "yogurt"], calories: 170, protein: 8, carbs: 28, fat: 3 },
  { keys: ["milk"], calories: 120, protein: 9, carbs: 12, fat: 5, basisMl: 250 },
  { keys: ["almonds"], calories: 580, protein: 21, carbs: 22, fat: 50, basisGrams: 100 },
  { keys: ["peanut butter"], calories: 95, protein: 4, carbs: 3, fat: 8 },
  { keys: ["chocolate piece", "piece of chocolate"], calories: 30, protein: 0, carbs: 4, fat: 2 },
  { keys: ["chocolate bar"], calories: 240, protein: 3, carbs: 28, fat: 14 },
  { keys: ["samosa"], calories: 260, protein: 6, carbs: 30, fat: 13 },
  ...
  { keys: ["ice cream scoop", "scoop of ice cream"], calories: 140, protein: 2, carbs: 18, fat: 7 },
  { keys: ["cookie"], calories: 150, protein: 2, carbs: 20, fat: 7 },
  { keys: ["biscuit"], calories: 70, protein: 1, carbs: 10, fat: 3 },
];
...
function quantityFor(segment, item) {
  ...
  const number = segment.match(/(?:^|\s)(\d+(?:\.\d+)?)\s*(?=[a-z])/i);
...
function estimateFood(text) {
  ...
  for (const segment of segments) {
    const matches = FOOD_DATABASE
      .map((item) => ({ item, key: item.keys.filter((key) => segment.includes(key)).sort((a, b) => b.length - a.length)[0] }))
      .filter((match) => match.key)
      .sort((a, b) => b.key.length - a.key.length);

    if (!matches.length) continue;
    const { item, key } = matches[0];
    const multiplier = quantityFor(segment, item);
    found.push({ item, key, multiplier });
  }
...
function SimpleHome({ app, onChange }) {
  ...
  const addEstimatedFood = (event) => {
    ...
    onChange({
      ...app,
      extraFoods: [...extras, { ...estimate, id: crypto.randomUUID(), estimated: true }],
    });
    setFoodText("");
  };
    ...
    <div className="simple-home-portal">
      <section className="simple-welcome">
        <div>
          ...
          <p>Select your usual meals. Add anything different below.</p>
        </div>
      ...
      <section className="simple-add-card">
        <div className="simple-add-heading">
          <div className="simple-add-icon"><Sparkles size={20} /></div>
          <div><h3>Add other food</h3><p>Type what you ate. Calories and macros are estimated automatically.</p></div>
        </div>
        <form onSubmit={addEstimatedFood} className="simple-food-form">
          <div className="simple-food-input-wrap">
            <Utensils size={18} />
            <input value={foodText} onChange={(event) => setFoodText(event.target.value)} placeholder="e.g. chicken burger and fries" />
            {estimate && <span className="simple-estimate">≈ {estimate.calories} kcal</span>}
          </div>
          <button disabled={!estimate} type="submit"><Plus size={18} /> Add</button>
        </form>
        {estimate?.confidence === "rough" && <p className="simple-estimate-note">No close food match was found, so this is a rough generic estimate. You can correct it on the Nutrition page.</p>}

        {extras.length > 0 && <div className="simple-extra-list">
          {extras.map((food) => <div key={food.id}><span><strong>{food.name}</strong><small>≈ {food.calories} kcal</small></span><button aria-label={`Remove ${food.name}`} onClick={() => removeExtra(food.id)}><Trash2 size={16} /></button></div>)}
        </div>}
      </section>
...
export default function SimpleHomeManager({ children }) {
